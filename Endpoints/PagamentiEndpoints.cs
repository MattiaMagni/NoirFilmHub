...
public static class PagamentiEndpoints
{
    public static RouteGroupBuilder MapPagamenti(this RouteGroupBuilder group)
    {
        ...
        group.MapGet("/esito", async (string session_id, ClaimsPrincipal user, FilmDbContext db, TicketPdfService pdfService, TicketEmailService emailService) =>
        {
            if (!TryGetUserId(user, out var userId))
            {
                return Results.Unauthorized();
            ...
            if (booking.Stato == "PendingStripe" && !string.IsNullOrWhiteSpace(stripeSecret))
            {
                try
                {
                    ...
                    {
                        await FinalizeBookingAsync(db, booking, requireLocks: false, pdfService, emailService);
        ...
        group.MapPost("/stripe/webhook", async (HttpRequest request, FilmDbContext db, TicketPdfService pdfService, TicketEmailService emailService) =>
        {
            if (string.IsNullOrWhiteSpace(stripeSecret))
            {
            ...
            if (stripeEvent.Type == EventTypes.CheckoutSessionCompleted)
            {
                ...
                var finalized = await FinalizeBookingAsync(db, booking, requireLocks: false, pdfService, emailService);
                if (!finalized)
                {
        ...
        TicketEmailService emailService)
    {
        ...
        try
        {
            ...
            _ = await emailService.SendTicketEmailAsync(booking.Utente.Email, booking.CodiceAcquisto, pdfBytes);
        }
        catch
        {
        }
